...
async function getTokenPriceFromMoralis(tokenAddress, symbol) {
  try {
    const res = await fetch(`https://deep-index.moralis.io/api/v2/erc20/${tokenAddress}/price`, {
      headers: {
        'X-API-Key': MORALIS_API_KEY
      }
    })
    const data = await res.json()
    return data?.usdPrice || TESTNET_PRICE_FALLBACK[symbol.toUpperCase()] || 0
  } catch (err) {
    console.warn(`[Moralis Price Error] ${symbol}:`, err.message)
...
export async function getWalletPnL(address, tokenSymbol) {
  ...
  try {
    ...
    for (const tx of transfers) {
      ...
      const price = await getTokenPriceFromMoralis(token.address, token.symbol)
      if (!price) continue
    ...
    const currentPrice = await getTokenPriceFromMoralis(token.address, token.symbol) || TESTNET_PRICE_FALLBACK[token.symbol.toUpperCase()] || 0
    ...
    console.warn(`[PnL Error] ${token.symbol}:`, err.message)
